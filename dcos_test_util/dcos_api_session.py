...
class ARNodeApiClientMixin:
    def api_request(self, method, path_extension, *, scheme=None, host=None, query=None,
        ...
        """
        if node is not None:
            assert self.masters is not None, 'Master list is unset!'
            assert self.all_slaves is not None, 'Agent list is unset!'
            assert node in self.masters or node in self.all_slaves, 'Selected node is not in cluster: {}. '\
                'Masters: {}, agents: {}'.format(node, self.masters, self.all_slaves)
...
class DcosApiSession(ARNodeApiClientMixin, RetryCommonHttpErrorsMixin, ApiClientSession):
    ...
    def _set_node_lists_if_unset(self):
        ...
        Thus, one may: (1) provide all of the nodes and the cluster will not be
        considered up until those nodes have been provide, (2) provide just one
        of the private agent, public agent, or the master group, (3) provide
        nothing at all and have the harness assume whatever is present is valid
        """
        if self.master_list is None:
            logging.info('Master list not provided, setting from exhibitor...')
        ...
        if self.slave_list is not None and self.public_slave_list is not None:
            # Return early if we do not need to set the other lists so that
            # we only need to call the mesos slave endpoint once
            return
        r = self.get('/mesos/slaves')
        r.raise_for_status()
        slaves_json = r.json()['slaves']
        if self.slave_list is None:
            logging.info('Private slave list not provided; fetching from mesos...')
        ...
        if self.public_slave_list is None:
            logging.info('Public slave list not provided; fetching from mesos...')
