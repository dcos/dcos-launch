...
5. If you have `jq` (CLI JSON parser) installed, you can grab the master node's IP with `dcos-launch describe | jq -r .masters[0].public_ip`. `describe` will get the node IP addresses of the cluster.

6. Try running the pytests in `dcos-integration-test` package that comes with DC/OS on your cluster. `dcos-launch pytest -- -s -vv test_composition.py` will run a test that checks that the cluster is up in verbose mode (-vv), including printing (-s) stdout. Note that if you are deploying an enterprise cluster, you will need to inject environment variables like `DCOS_LOGIN_UNAME=thelogin DCOS_LOGIN_PW=thepassword dcos-launch pytest -e DCOS_LOGIN_UNAME,DCOS_LOGIN_PW -- -vv`.

7. To ssh into your cluster, you need the SSH key. If you need to get the one generated by the key_helper, use `cat cluster_info.json | jq -r .ssh_private_key > ssh_key; chmod 600 ssh_key`

8. Run `dcos-launch delete` to delete your cluster.
